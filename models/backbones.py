...
class DilatedResNetBackbone(nn.Module):
    def __init__(self, block, layers, strides=[1,2,2,2], dilations=[1,1,1,1], pretrained=None):
        ...
        """
        # Load the pretrained weights, excluding the new layer
        pretrained = {k: v for k, v in pretrained.items() if 'adjust_channels' not in k}
        rnet.load_state_dict(pretrained, strict=False)
        """
        self.rnet = rnet
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            x = self.rnet(x)
        """
        # Additional tensor with shape (batch_size, 2, height, width)
        additional_tensor = torch.ones((x.size(0), 2, x.size(2), x.size(3)), device=x.device, dtype=x.dtype)
