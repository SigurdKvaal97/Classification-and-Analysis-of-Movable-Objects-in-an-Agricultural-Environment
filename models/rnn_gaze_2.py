...
class FrameLSTM(nn.Module):
    ...
    def embed_clip(self, frame_feats, **kwargs):
        # Ensure lengths is a CPU tensor of type int64
    ...
    def anticipation_loss(self, frame_feats, lstm_feats, batch, gaze_feats):
        B = frame_feats.shape[0]
        ...
        active_gaze_feats = gaze_feats[torch.arange(B), frame_idx]
        gaze_pooled = self.pool(active_gaze_feats).view(B, -1)

        active_pooled = torch.cat((active_pooled, gaze_pooled), dim=1) 
        ...
        positive_zeros = torch.zeros_like(positive_pooled)
        
        
        positive_pooled = torch.cat((positive_pooled, positive_zeros), dim=1) 
        ...
        if self.ant_loss=='mse':
            ...
            negative_zeros = torch.zeros_like(negative_pooled)
            negative_pooled = torch.cat((negative_pooled, negative_zeros), dim=1) 
    ...
    def forward(self, batch, **kwargs):
        frames, length, gaze_frames = batch['frames'], batch['length'], batch["gaze_frames"]
        B, T = frames.shape[:2]
        S = self.spatial_dim
     
        frame_feats = self.flatten_apply(frames, lambda t: self.backbone(t)) # (B, T, 256, 28, 28)

        frame_feats_gaze = self.flatten_apply(gaze_frames, lambda t: self.backbone(t)) # (B, T, 256, 28, 28)

        pool_feats = self.flatten_apply(frame_feats, lambda t: self.pool(t)).view(B, T, -1) # (B, T, 256)
        pool_feats_gaze = self.flatten_apply(frame_feats_gaze, lambda t: self.pool(t)).view(B, T, -1) # (B, T, 256)

        combined_feats = torch.cat([pool_feats, pool_feats_gaze], dim=-1) # (B, T, 512)

        clip_feats, lstm_feats = self.embed_clip(combined_feats, length=length) # (B, 2048)
        ...
        loss_dict.update(self.anticipation_loss(frame_feats, lstm_feats, batch, frame_feats_gaze))
    ...
    def ic_classifier(self, frame_feats, **kwargs):
        ...
        #frame_feats = torch.cat((frame_feats, zeros_tensor), dim=1)
        #------------------
        ...
        zeros_tensor = torch.zeros_like(pool_feats)
        
        pool_feats = torch.cat((pool_feats, zeros_tensor), dim=1)
