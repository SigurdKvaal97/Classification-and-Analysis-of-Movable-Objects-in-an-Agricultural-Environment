...
class PairedTransform:
    ...
    def val_transform(self, image, heatmap):
        ...
        # Scale the normalized heatmap to the range 0-255 and convert to uint8
        heatmap_scaled = (heatmap_normalized * 255).astype(np.uint8)
        
        # Convert the scaled heatmap to an image
        heat = Image.fromarray(heatmap_scaled, mode='L')  # 'L' mode for grayscale
        print("yayy")
        # Save the heatmap image
        heat.save(f"heat_im_test_{image}.jpg")

        # If image is a PIL Image object, save it
        image.save("image_im_test2.jpg")
        """
