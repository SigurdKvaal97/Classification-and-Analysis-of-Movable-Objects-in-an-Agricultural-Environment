...
def save(epoch):
    ...
    torch.save(checkpoint, '%s/MastetData_test_gazepoint_img_all_ckpt_E_%d.pth'%(args.cv_dir, epoch))
...
if args.dset=='opra':
    ...
    ant_loss = 'triplet'
    valset = epic.EPICInteractions(root=data._DATA_ROOTS[args.dset], split='val', max_len=args.max_len)
...
valloader =  torch.utils.data.DataLoader(valset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, sampler=valset.data_sampler())
